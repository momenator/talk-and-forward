"""Invoke with `fastapi dev --host 127.0.0.1 --port 5555 main.py`"""
...
def get_matches(conversationId: str):
    ...
    for item in club_names:
        try:
            out.append({"name": item["name"], "category": club_to_cat[item["name"]], "description": descriptions_dict[item["name"]], "reason": item["reason"]})
        except:
            pass
